...
#### 3. Inference y Diagnósticos

- Integración directa de `confidence_intervals` en `GLMResult.predict(interval="confidence")`.
- Extensión de `wald_test` a contrastes multivariados + pruebas LRT.
- Incorporación de residuales studentizados, DFBETAs y resúmenes tabulares en `glm_diagnostics`.
- Nuevos ensayos unitarios en `tests/test_models/test_glm_fitting.py` y `tests/test_inference/test_diagnostics.py`.

#### 4. Métricas y Validación

- Añadir `concordance_index`, deviance específica y variantes avanzadas de pseudo R² en `aurora/validation/metrics`.
- Introducir `StratifiedKFold` y herramientas de scoring ponderado en `aurora/validation/cross_val`.
- Garantizar cobertura de pruebas en `tests/test_validation` y ejemplos referenciados en notebooks.
...
| --- | --- | --- |
| **Inferencia** | Integrar intervalos y p-values en `GLMResult`, ampliar `wald_test` a contrastes multivariados y LRT. | `predict(interval="confidence")` funcional; pruebas reforzadas en `tests/test_models/test_glm_fitting.py` y `tests/test_inference/test_hypothesis.py`. |
...
- GitHub Projects Kanban: columnas `Todo → In progress → Review → Done`.
- Issues con etiquetas de categoría, tamaño (`S/M/L`) y responsable; checklist de definición de terminado.
- Repaso semanal de backlog + retrospectiva corta para riesgos.

### Ciclo QA semanal

- Pipeline automatizado: `pytest`, suites específicas, comparaciones vs statsmodels/R (`benchmarks/run_glm_checks.py`).
- Métricas mínimas: cobertura ≥85%, error relativo en coeficientes <1e-6, medición de tiempos NumPy/PyTorch.
- Reporte en `docs/reports/QA_SEMANA.md` con hallazgos y acciones correctivas.

### Hitos Sprint (3 semanas)

1. **Semana 1**: Cerrar backlog de inferencia y activar validación externa.
2. **Semana 2**: Entregar diagnósticos y métricas avanzadas + `GLMResult.summary()` inicial.
3. **Semana 3**: Publicar documentación/notebooks y prototipo de `plot_diagnostics`.

### Preparativos Release 0.2.0

- Checklist funcional completado (inferencias, diagnósticos, métricas, documentación).
- Changelog actualizado y sincronización de versión en archivos clave.
- Resultados comparativos NumPy/PyTorch publicados en repositorio.
...
> Consulta `aurora/smoothing/DESIGN.md` para la arquitectura detallada, plan incremental y riesgos abiertos del módulo de smoothing.
