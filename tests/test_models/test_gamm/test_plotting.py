...
def test_caterpillar_plot_visual_elements(simple_gamm_result):
    ...
    vlines = [
        ...
        if hasattr(line, "get_xdata") and len(line.get_xdata()) > 0 and line.get_xdata()[0] == 0
    ]
    ...
    has_ci = len([c for c in ax.collections if isinstance(c, LineCollection)]) > 0 or any(
        hasattr(line, "get_ydata") and len(line.get_ydata()) == 2 for line in ax.lines
    )
...
def test_residuals_plot_reference_line(simple_gamm_result):
    ...
    hlines = [
        line for line in ax.lines if len(set(line.get_ydata())) == 1 and line.get_ydata()[0] == 0
    ]
...
def test_fitted_vs_observed_diagonal(simple_gamm_result):
    ...
    diagonal_lines = [
        line
        for line in ax.lines
        if len(line.get_xdata()) == 2 and np.allclose(line.get_xdata(), line.get_ydata())
    ]
