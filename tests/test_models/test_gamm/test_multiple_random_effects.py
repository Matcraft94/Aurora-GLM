...
def nested_hierarchical_data():
    ...
    for school in range(n_schools):
        for cls_within_school in range(n_classes_per_school):
...
def test_nested_random_effects_three_levels(nested_hierarchical_data):
    ...
    beta_0_est = result.beta_parametric[0]
    assert 65 < beta_0_est < 75, (
        f"Intercept should be near 70, got {beta_0_est:.2f}"
    )
    ...
    print(f"  Class: true={true_params['sigma2_class']:.2f}, "
    ...
    total_variance_true = (true_params['sigma2_school'] + 
                          true_params['sigma2_class'] + 
                          true_params['sigma2_student'])
...
def test_three_level_model_with_formula():
    ...
    n_total = n_cities * n_obs_per_city
    ...
    for country in range(n_countries):
        for region_within_country in range(n_regions_per_country):
            for city_within_region in range(n_cities_per_region):
                for obs in range(n_obs_per_city):
                    y = (beta_0 + 
...
def test_reml_convergence_multiple_terms(simple_crossed_data):
    ...
    assert not np.isnan(result.log_likelihood), (
        "Log-likelihood should not be NaN"
    )
    ...
    assert result.bic > result.aic, (
        "BIC should be larger than AIC (penalizes complexity more)"
    )
...
def test_multiple_random_effects_summary():
    ...
    # Fit model
    result = fit_gamm(
        formula='y ~ 1 + (1 | group_a) + (1 | group_b)',
        data=df,
        covariance='identity'
    )
