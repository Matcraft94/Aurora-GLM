...
try:
    ...
    from aurora.models.gamm.diagnostics import (  # noqa: F401
...
try:
    ...
    if isinstance(random_effects_obj, dict):
        ...
        random_effects_array = random_effects_obj.get(
            0, list(random_effects_obj.values())[0] if random_effects_obj else None
        )
    ...
    if random_effects_array is not None:
        residuals_school_intercepts = (
            random_effects_array[:, 0] if random_effects_array.ndim > 1 else random_effects_array
        )
        residuals_school_slopes = (
            random_effects_array[:, 1] if random_effects_array.ndim > 1 else None
        )
    else:
    ...
    print(
        f"✓ School random effects (intercepts): n={len(residuals_school_intercepts)}, mean={residuals_school_intercepts.mean():.4f}"
    )
    if residuals_school_slopes is not None:
        print(
            f"✓ School random effects (slopes): n={len(residuals_school_slopes)}, mean={residuals_school_slopes.mean():.4f}"
        )
